...
/*Process 0 sends very first message, then waits to receive the "echo" that has gonethe world.*/
if ( id == 0 )
{
dest = 1;
source = p - 1;
...
{
/*Just in case, set the entries of X in a way that identifies which iteration of the test is being
carried out.*/
for ( j = 0; j < n; j++ )
{
x[j] = ( double ) ( test + j );
